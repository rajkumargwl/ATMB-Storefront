...
export default function Footer({ data }: FooterProps) {
  ...
  return (
    <footer className="bg-[#272B30] md:bg-PrimaryBlack text-white px-5">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 md:gap-5">
          ...
          <div className="lg:col-span-2 md:max-w-[306px] flex items-center md:items-start flex-col justify-between gap-6">
            <div className="block">
              ...
              {data.description && (
                <p className="text-LightWhite text-[16px] leading-[24px] font-normal tracking-[0px] text-center md:text-left">
                  {data.description}
                </p>
              )}
            </div>
            ...
            {data.appButtons && (
              <div className="flex gap-3 justify-center md:justify-start">
                ...
                <a className="rounded-[100px] bg-[#F60] font-Roboto text-white px-4 py-3 font-normal leading-[16px] tracking-[0.08px] text-base flex items-center gap-3 h-[52px]" href="/"><CallWhite /> +1 (775) 500 0579</a>
              </div>
            )}
          </div>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-x-4 gap-y-6 md:gap-14 lg:col-span-3 w-full xl:min-w-[812px] xl:-ml-5">
            ...
            {data.companyColumn && (
              <div>
                <h4 className="text-[18px] leading-[27px] font-normal text-white mb-3 md:mb-4 tracking-[0px] font-Roboto">
                ...
                <ul className="space-y-1">
                  {data.companyColumn.links?.map((link, idx) => (
                    <li key={idx}>
                      <Link
                        to={link.link || '#'}
                        className="text-LightWhite text-[14px] leading-[21px] font-normal py-1 inline-block tracking-[0px] font-Roboto"
                      >
                        {link.label}
                      </Link>
                    </li>
                ...
                <ul className="space-y-1">
                  {data.servicesColumn.links?.map((link, idx) => (
                    <li key={idx}>
                      <Link
                        to={link.link || '#'}
                        className="text-LightWhite text-[14px] leading-[21px] font-normal py-1 tracking-[0px] font-Roboto inline-block"
                      >
                        {link.label}
                      </Link>
                    </li>
                  ...
                  {data.locationsColumn.links?.map((link, idx) => (
                    <li key={idx}>
                      <Link
                        ...
                        className={`text-sm ${
                          link.highlight
                            ? 'text-LightWhite text-[14px] leading-[21px] font-normal py-1 tracking-[0px] font-Roboto inline-block'
                            : 'text-LightWhite text-[14px] leading-[21px] font-normal py-1 tracking-[0px] font-Roboto inline-block'
                        }`}
                      >
                        {link.label}
            ...
            {/* {data?.contactColumn && ( */}
              <div>
                ...
                <p className='text-LightWhite text-[14px] leading-[21px] font-normal py-1 tracking-[0px] font-Roboto inline-block'>{data?.contactColumn?.address}</p>
              </div>
            {/* )} */}
          </div>
        </div>
      ...
      <div className="max-w-[1312px] mx-auto flex flex-col md:flex-row items-center justify-between">
        ...
        <div className="flex space-x-5 md:space-x-11 mb-4 md:mb-0">
          <a href="#" className="text-[18px] tracking-[0px] leading-[27px] font-normal text-LightWhite underline decoration-solid">Instagram</a>
          <a href="#" className="text-[18px] tracking-[0px] leading-[27px] font-normal text-LightWhite underline decoration-solid">Facebook</a>
          <a href="#" className="text-[18px] tracking-[0px] leading-[27px] font-normal text-LightWhite underline decoration-solid">Youtube</a>
        </div>

        {/* Right - App info */}
        <div className="flex items-center space-x-4 flex-col-reverse md:flex-row">
          
            <span className="text-[14px] tracking-[0px] leading-[21px] font-normal text-white inline mt-4 md:mt-[0px]">
        ...
        <div className="max-w-[1312px] mx-auto pt-5 pb-6 flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap justify-center gap-4 md:gap-6">
            {data.bottomLinks?.map((link, idx) => (
              <Link key={idx} to={link.link || '#'} className="text-[14px] leading-[21px] font-normal tracking-[0px] text-white">
                {link.label}
              </Link>
            ))}
          </div>
          ...
          <div className="flex items-center gap-4">
            {/* {data.socialLinks?.map((social, idx) => (
             ...
             <span className="text-[14px] leading-[21px] tracking-[0px] font-Roboto font-normal text-white inline">
