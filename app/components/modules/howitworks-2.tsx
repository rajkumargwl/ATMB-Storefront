...
export function HowItWorks2({ data }: HowItWorks2Props) {
  return (
    <section className="bg-PrimaryBlack py-[40px] md:py-[60px] lg:py-[100px] px-5 relative z-[0] overflow-hidden">
      ...
      <div className="absolute flex md:hidden z-[1] top-[0px] right-[0px]">
        <svg xmlns="http://www.w3.org/2000/svg" width="393" height="485" viewBox="0 0 393 485" fill="none">
          <g filter="url(#filter0_f_1664_59927)">
            <path d="M297.881 46.2699C287.099 51.9219 280.495 28.0277 268.786 24.7039C256.853 21.3166 243.913 24.1009 232.004 27.5698C217.506 31.7926 202.091 36.3231 191.594 47.1879C180.942 58.2117 162.999 77.042 172.863 88.7774C189.962 109.119 230.216 93.9963 247.734 113.977C257.288 124.875 230.748 138.423 229.008 152.817C227.04 169.113 220.911 202.552 237.227 200.944C262.574 198.446 267.924 159.035 290.26 146.782C297.576 142.769 304.013 155.857 310.944 160.506C326.834 171.163 341.265 201.319 358.141 192.306C374.214 183.722 354.288 156.503 352.405 138.363C351.509 129.723 348.92 121.285 349.873 112.652C350.667 105.461 352.661 97.9237 357.465 92.5192C375.283 72.4744 413.986 65.0582 415.966 38.2985C417.328 19.889 377.642 37.1477 362.32 26.8774C349.656 18.3883 354.593 -17.0168 339.998 -12.6243C316.704 -5.61429 319.43 34.9746 297.881 46.2699Z" fill="#FF6600"/>
          </g>
          <defs>
            <filter id="filter0_f_1664_59927" x="-114" y="-297" width="814" height="782" filterUnits="userSpaceOnUse" color-interpolation-filters="sRGB">
              <feFlood flood-opacity="0" result="BackgroundImageFix"/>
              <feBlend mode="normal" in="SourceGraphic" in2="BackgroundImageFix" result="shape"/>
              <feGaussianBlur stdDeviation="142" result="effect1_foregroundBlur_1664_59927"/>
            </filter>
          </defs>
        </svg>
      </div>
      ...
      <div className="max-w-[1236px] mx-auto relative z-[2]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-[44px] md:mb-[64px]">
          {data.features?.map((feature, index) => (
            <div
              key={index}
              className="bg-white text-left p-6 rounded-[12px] flex flex-col gap-10 justify-between  overflow-hidden"
            >
                ...
                <div className="flex items-center justify-center">
                  <img
                    ...
                    alt={feature.title || `Feature ${index + 1}`}
                     className={`w-auto h-auto object-cover ${index !== 0 ? 'mb-[-24px]' : ''}`}
                  />
                </div>
              )}
            </div>
        ...
        {data.button && (
          <div className="flex items-center justify-center">
            <Link
              to={data.button.url || '#'}
              className="group relative flex items-center justify-center w-full md:w-[312px] bg-DarkOrange text-white font-Roboto font-normal leading-[16px] text-[16px] tracking-[0.08px] h-[52px] px-[16px] py-[12px] rounded-[100px] overflow-hidden transition-all hover:scale-[1.01] hover:bg-[#DD5827]"
            >
