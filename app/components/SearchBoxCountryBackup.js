...
export default function SearchBoxCountry({
  ...
  console.log("SearchBoxCountry rendered with results:", results);
...
//old filtering logic
  ...
  //     [item.addressLine1, item.city, item.postalCode, item.state, item.country] 
  //       .filter(Boolean)
...
//new filtering logic
const query = searchquery.toLowerCase().replace(/\s*,\s*/g, ","); // normalize spaces around commas

const filtered = results.filter((item) => {
  // Combine all fields into one normalized address string
  const fullAddress = [
    item.addressLine1,
    item.city,
    item.state,
  ...
  // ✅ Allow match if query is a substring or partial match of any comma-separated part
  if (fullAddress.includes(query)) return true;

  // ✅ Also split user input by commas and require all segments to exist in address
  const queryParts = query.split(",").map((p) => p.trim()).filter(Boolean);
  return queryParts.every((part) => fullAddress.includes(part));
});
            ...
            <ul className="max-h-72 overflow-y-auto space-y-6">
              {qresults.length > 0 ? (
                qresults.map((item) => (
                  ...
                  >
                    <span className="mr-2 font-medium text-PrimaryBlack hover:text-[#ff6600]">{item.addressLine1},</span>
