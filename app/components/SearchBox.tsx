...
export default function SearchBox({
  ...
  // 🔍 Enhanced search — supports country, state, city, address filtering
  useEffect(() => {
    if (!searchquery.trim()) {
      setQresults([]);
      return;
    ...
    const stateMatches = filtered.filter(
      (i) => !countryMatches.includes(i) && i.state?.toLowerCase().includes(query)
    );
    ...
    // Auto-navigation for exact country/state matches
  ...
  // 🔁 Optional: URL sync with debounce
  useEffect(() => {
